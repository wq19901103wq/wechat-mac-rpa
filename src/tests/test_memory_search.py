...
def engine(tmp_path):
    ...
    导致测试用名被 _resolve_alias 改写（如'示例用户乙'被规范成'示例用户乙-远舟'，
    使断言'【示例用户乙的记忆】'对不上实际'【示例用户乙-远舟的记忆】'）。需要别名的
...
class TestPrimaryMatching:
    ...
    def test_exact_name_match_is_primary(self, engine):
        ...
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 职业：算法工程师\n- 爱好：股票")
        write_user_wiki(engine, "示例用户乙", "- 与示例用户甲：同事")
        write_noise_user(engine, "噪声用户")

        result = engine.search_keyword("示例用户甲", max_chars=10000)

        assert "【示例用户甲的记忆】" in result
        assert "- 职业：算法工程师" in result
        # primary 必须排在第一位
        lines = [line for line in result.split("\n") if line.startswith("【")]
        assert lines[0].startswith("【示例用户甲的记忆】")

    def test_only_one_primary_allowed(self, engine):
        """最多只允许 1 个 primary，其余同名/子串文档按 BM25 正常排序。"""
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 职业：算法工程师")
        write_user_wiki(engine, "示例用户甲_2", "- 姓名：示例用户甲_2\n- 详细描述：" + "x" * 2000)
        write_user_wiki(engine, "示例用户甲、示例用户午、示例用户甲", "- 包含示例用户甲的文件名，内容很短")
        write_user_wiki(engine, "示例用户乙", "- 与示例用户甲：同事")
        write_noise_user(engine, "噪声用户")

        result = engine.search_keyword("示例用户甲", max_chars=10000)

        # 只能有 1 个 primary → 示例用户甲置顶
        lines = [line for line in result.split("\n") if line.startswith("【")]
        assert lines[0].startswith("【示例用户甲的记忆】")

        # 示例用户甲的完整 wiki 只应出现一次（primary 的完整内容）
        assert result.count("【示例用户甲的记忆】") == 1

    def test_group_never_primary(self, engine):
        """群 wiki 永远不可能是 primary。"""
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 职业：算法工程师")
        write_group_wiki(engine, "示例用户甲的家人群", "- 群成员：示例用户甲、示例用户丙")
        write_noise_user(engine, "噪声用户")

        result = engine.search_keyword("示例用户甲", max_chars=10000)

        assert "【示例用户甲的家人群群记忆】" in result
...
class TestTopNExpansion:
    ...
    def test_top10_includes_more_non_primary(self, engine):
        ...
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 职业：算法工程师")
        for i in range(8):
            write_user_wiki(engine, f"同事{i}", f"- 与示例用户甲：同事，编号{i}\n- 详细描述：{'工作内容' * 50}")
        write_noise_user(engine, "噪声用户")

        result = engine.search_keyword("示例用户甲 同事", max_chars=100000)
    ...
    def test_primary_still_first(self, engine):
        ...
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 职业：算法工程师")
        for i in range(8):
            write_user_wiki(engine, f"同事{i}", "- 与示例用户甲：同事")
        write_noise_user(engine, "噪声用户")

        result = engine.search_keyword("示例用户甲", max_chars=100000)
    ...
    def test_max_chars_respected(self, engine):
        ...
        result = engine.search_keyword("示例用户甲 同事", max_chars=2000)
...
class TestAliasScoring:
    ...
    def test_expanded_aliases_downweighted(self, engine):
        ...
        # 示例用户甲 wiki 里有 "示例别名甲" 别名
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 别名：示例别名甲\n- 职业：算法")
        # 示例用户卯 wiki 里大量提到 "示例别名甲"（模拟真实场景），但和"同事"无关
        write_user_wiki(engine, "示例用户卯", "- 朋友：示例别名甲\n- 示例别名甲示例别名甲示例别名甲示例别名甲\n-  unrelated content")
        # 示例用户乙 wiki 里有 "示例用户甲" 和 "同事"，但没有 "示例别名甲"
        write_user_wiki(engine, "示例用户乙", "- 与示例用户甲：同事")
        # Alex wiki 里有 "示例用户甲" 和 "同事"
        write_user_wiki(engine, "Alex", "- 与示例用户甲：同事")
        # 给 aliases.json 注入别名，让 engine 扩展出 "示例别名甲"
        engine._aliases["示例用户甲"] = ["示例别名甲"]
        for i in range(3):
            write_user_wiki(engine, f"路人{i}", "- 无关内容")

        result = engine.search_keyword("示例用户甲 同事", max_chars=10000)

        # 示例用户乙和 Alex 应被召回（因为他们有"同事"关系）
        assert "【示例用户乙的记忆】" in result or "【示例用户乙的记忆】…" in result
        assert "【Alex的记忆】" in result or "【Alex的记忆】…" in result

    def test_alias_group_shares_tfidf(self, engine):
        """所有扩展别名应共享一组 TF-IDF，而不是每个别名单独计算。"""
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 别名：示例别名甲、关系别名甲")
        # 文档A：包含 "示例别名甲"（示例用户甲别名）
        write_user_wiki(engine, "文档A", "- 提到示例别名甲")
        # 文档B：包含 "关系别名甲"（示例用户甲别名）
        write_user_wiki(engine, "文档B", "- 提到关系别名甲")
        # 文档C：包含 "示例用户甲"（原始搜索词）
        write_user_wiki(engine, "文档C", "- 提到示例用户甲")
        engine._aliases["示例用户甲"] = ["示例别名甲", "关系别名甲"]
        ...
        # 文档A、B、C 都应被召回（因为它们都提到了示例用户甲的某个别名或本名）
...
class TestCrossPersonRelationship:
    ...
    def test_colleague_search_recalls_others(self, engine):
        """搜'示例用户甲 同事'应召回示例用户乙、Alex 等包含关系描述的文档。"""
        write_user_wiki(engine, "示例用户甲", "- 姓名：示例用户甲\n- 职业：算法工程师\n- 工作经历：公司甲→公司乙")
        write_user_wiki(engine, "示例用户乙", "- 示例用户甲（关系别名甲）：同事（公司乙）、朋友")
        write_user_wiki(engine, "Alex", "- 与示例用户甲：同事（同一组）、朋友")
...
class TestAliasExtractionSanitization:
    """别名提取的拆分 + 脏数据过滤（修复'示例别名辛'召回失败）。"""

    def test_user_wiki_alias_string_split(self, engine):
        """## 别名 段落里的 '示例别名庚、示例别名辛' 必须拆成两条，不能整串入库。"""
        wiki = (
            "# 示例用户玄\n\n## 别名\n- 示例别名庚、示例别名辛\n\n## 基本信息\n- 职业：测试\n"
        )
        aliases = engine._extract_aliases_from_user_wiki(wiki, "示例用户玄")
        assert aliases == ["示例别名庚", "示例别名辛"]
    ...
    def test_role_blacklist_rejected(self, engine):
        ...
        aliases = engine._extract_aliases_from_user_wiki(wiki, "示例用户甲")
        assert aliases == ["真别名"]
    ...
    def test_search_recalls_via_split_alias(self, engine, tmp_path):
        """端到端：aliases.json 存了整串 '示例别名庚、示例别名辛'，搜 '示例别名辛' 仍能召回本人。"""
        ...
        engine._aliases = {}
        write_user_wiki(engine, "示例用户玄", "- 姓名：示例用户玄\n- 爱好：干脆面")
        write_noise_user(engine, "噪声用户")
        # 模拟历史脏数据：整串未拆分，经 merge 清洗后应拆成两条
        engine._merge_aliases("示例用户玄", ["示例别名庚、示例别名辛"])
        assert engine._aliases["示例用户玄"] == ["示例别名庚", "示例别名辛"]
        result = engine.search_keyword("示例别名辛", max_chars=10000)
        assert "【示例用户玄的记忆】" in result
    ...
    def test_sanitize_wiki_aliases_removes_cross_person_alias(self, engine):
        ...
        cleaned = engine._sanitize_wiki_aliases(wiki, "示例用户宇")

        assert "示例别名乙" not in cleaned
        assert "示例用户寅" in cleaned
    ...
    def test_sanitize_wiki_aliases_resolves_filename_alias(self, engine):
        ...
        cleaned = engine._sanitize_wiki_aliases(wiki, "示例用户寅")
    ...
    def test_sanitize_wiki_aliases_keeps_group_wiki_aliases(self, engine):
        ...
        assert "示例别名乙" in cleaned
        assert "示例用户寅" in cleaned

    def test_sanitize_wiki_aliases_preserves_alias_with_source_annotation(self, engine):
        """带 （来源：...） 注解的有效别名不应被误清洗。"""
        engine._aliases = {"示例用户甲": ["示例别名乙"]}
        wiki = (
            "# 示例用户甲\n\n## 别名\n"
            "- 别名：Co总（来源：2025-01-01 群聊，示例用户天、示例用户地、bot 均如此称呼）、扛把子\n"
        )

        cleaned = engine._sanitize_wiki_aliases(wiki, "示例用户甲")
...
class TestNormalizeChatName:
    ...
    def test_strip_emoji_and_fold_space(self):
        ...
        # emoji 剥离
        assert normalize_chat_name("🎀示例别名己🎀") == "示例别名己"
        assert normalize_chat_name("Nico. 🤍") == "Nico."
        # 空格折叠
        assert normalize_chat_name("示例群名甲  周一至周五") == "示例群名甲 周一至周五"
...
class TestLintMemory:
    ...
    def test_lint_detects_bloated_and_duplicates(self, engine, tmp_path):
        ...
        # 重复（归一化后同名）
        write_user_wiki(engine, "🎀示例别名己🎀", "内容")
        write_user_wiki(engine, "示例别名己", "内容")
        ...
        # 示例别名己 / 🎀示例别名己🎀 归一化后都是 "示例别名己"
        dup_found = any("示例别名己" in d.get("normalized", "") for d in report["duplicates"])
        assert dup_found

    def test_lint_detects_alias_conflicts(self, engine, tmp_path):
        engine._aliases = {"示例用户甲": ["示例别名甲"], "示例用户丑": ["示例别名甲"]}
        report = engine.lint_memory()
        conflicts = [c for c in report["conflicts"] if c["alias"] == "示例别名甲"]
        assert len(conflicts) == 1
        assert set(conflicts[0]["mains"]) == {"示例用户甲", "示例用户丑"}
...
class TestLLMRerank:
    ...
    def test_rerank_relation_disambiguation(self, engine):
        """关系消歧场景：搜'示例用户甲妈妈'，LLM 应把母亲(示例用户午)排到岳母(示例用户未)前面。

        模拟 BM25 把岳母排前（词面'妈妈'匹配'示例关系称呼甲'），LLM 理解语义后翻转。
        """
        engine._aliases = {}
        # 模拟 wiki 内容
        write_user_wiki(engine, "示例用户午", "- 母亲：示例用户子（示例用户甲的母亲）")
        write_user_wiki(engine, "示例用户未 示例关系称呼甲", "- 示例用户丙的妈妈（岳母，非示例用户甲母亲）")
        write_noise_user(engine, "噪声用户")

        # LLM 理解"示例用户甲妈妈=母亲"语义，把示例用户午（编号2）排前
        class _SmartLLM:
            def chat(self, messages=None, tools=None, temperature=None, max_tokens=None, timeout=None):
                # 候选顺序：示例用户未(1, BM25高分) 示例用户午(2)，LLM 翻转
                return "[2, 1]"

        engine.llm_client = _SmartLLM()
        result = engine.search_keyword("示例用户甲 妈妈", max_chars=5000)
        # 示例用户午应排在示例用户未前面
        idx_qiushui = result.find("【示例用户午的记忆】")
        idx_fengye = result.find("【示例用户未 示例关系称呼甲的记忆】")
        assert idx_qiushui != -1, "示例用户午应被召回"
        assert idx_qiushui < idx_fengye, "母亲(示例用户午)应排在岳母(示例用户未)前面"
...
class TestAliasConflictGuard:
    ...
    def test_reject_alias_already_owned_by_another_user(self, engine, tmp_path):
        ...
        engine.overrides_dir = tmp_path / "overrides"
        engine.overrides_dir.mkdir(parents=True, exist_ok=True)
        engine._aliases = {"示例用户甲": ["示例别名壬"], "示例用户宇": []}

        engine._do_merge_aliases("示例用户宇", ["示例别名壬"])

        # 示例别名壬 仍只属于 示例用户甲
        assert engine._aliases["示例用户甲"] == ["示例别名壬"]
        assert "示例别名壬" not in engine._aliases["示例用户宇"]

    def test_allow_alias_for_same_user(self, engine, tmp_path):
        """同一用户重复合并同一别名不应重复添加。"""
        engine.overrides_dir = tmp_path / "overrides"
        engine.overrides_dir.mkdir(parents=True, exist_ok=True)
        engine._aliases = {"示例用户甲": ["示例别名壬"]}

        engine._do_merge_aliases("示例用户甲", ["示例别名壬", "示例别名丙"])

        assert engine._aliases["示例用户甲"] == ["示例别名壬", "示例别名丙"]
    ...
    def test_persisted_aliases_json_no_conflict(self, engine, tmp_path):
        ...
        aliases_path.write_text(
            '{"users": {"示例用户甲": {"aliases": ["示例别名壬"], "notes": ""}, '
            '"示例用户宇": {"aliases": [], "notes": ""}}}',
            encoding="utf-8",
        )
        engine._aliases = {
        ...
        engine._do_merge_aliases("示例用户宇", ["示例别名壬", "示例别名癸"])

        data = json.loads(aliases_path.read_text(encoding="utf-8"))
        assert "示例别名壬" not in data["users"]["示例用户宇"]["aliases"]
        assert "示例别名癸" in data["users"]["示例用户宇"]["aliases"]
...
class TestAliasExtractionOwnership:
    ...
    def test_user_wiki_skips_alias_owned_by_other(self, engine):
        ...
        aliases = engine._extract_aliases_from_user_wiki(wiki, "示例用户宇")

        assert "示例别名乙" not in aliases
        assert "示例用户寅" in aliases  # 属于自己，允许再次出现
    ...
    def test_user_wiki_keeps_alias_owned_by_same_user(self, engine):
        ...
        assert "示例别名乙" in aliases  # 属于自己，允许再次出现
        assert "示例别名丙" in aliases
    ...
    def test_group_wiki_skips_alias_owned_by_other(self, engine):
        ...
        wiki = "## 群成员画像\n**示例用户甲（示例别名乙/示例别名丙）**\n**示例用户宇（示例别名乙/示例用户寅/示例别名癸）**\n"

        result = engine._extract_aliases_from_group_wiki(wiki)

        assert "示例别名乙" not in result.get("示例用户宇", [])
        assert "示例用户寅" in result.get("示例用户宇", [])
        assert "示例别名癸" in result.get("示例用户宇", [])
        assert "示例别名乙" in result.get("示例用户甲", [])
    ...
    def test_group_wiki_resolves_main_name(self, engine):
        ...
        # 应归一化为 示例用户宇，且示例别名乙已属于示例用户甲，应被跳过
        assert "示例别名乙" not in result.get("示例用户宇", [])
    ...
    def test_group_wiki_keeps_alias_for_same_owner(self, engine):
        ...
        assert "示例别名乙" in result.get("示例用户甲", [])
        assert "示例别名丙" in result.get("示例用户甲", [])
