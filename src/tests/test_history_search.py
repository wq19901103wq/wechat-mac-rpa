...
def _make_index():
    ...
    messages = [
        {
            "id": "f_0", "text": "明天去上海吗", "sender": "示例用户丁",
            "is_self": False, "chat_name": "示例用户丁", "chat_type": "single",
            "context_ids": ["f_0", "f_1"],
        },
        {
            "id": "f_1", "text": "嗯去出差", "sender": "示例用户丁",
            "is_self": True, "chat_name": "示例用户丁", "chat_type": "single",
            "context_ids": ["f_0", "f_1"],
        },
        {
            "id": "g_0", "text": "那家日料不错", "sender": "示例用户戌",
    ...
    idx.sender_index = {"示例用户丁": ["f_0", "f_1"], "示例用户戌": ["g_0"]}
...
def _make_near_tied_index():
    """f_0(示例用户丁) 与 g_0(示例用户戌) 近似平手：cos 0.98 vs 0.97，+0.05 加权可翻转。"""
    idx = HistorySearchIndex.__new__(HistorySearchIndex)
    messages = [
        {
            "id": "f_0", "text": "A", "sender": "示例用户丁", "is_self": False,
            "chat_name": "示例用户丁", "chat_type": "single", "context_ids": ["f_0"],
        },
        {
            "id": "g_0", "text": "B", "sender": "示例用户戌", "is_self": False,
    ...
    idx.msg_by_id = {m["id"]: m for m in messages}
    idx.id_to_idx = {m["id"]: i for i, m in enumerate(messages)}
    idx.sender_index = {"示例用户丁": ["f_0"], "示例用户戌": ["g_0"]}
...
class TestSearchLogic:
    ...
    def test_sender_boost(self):
        ...
        # 给示例用户戌加权 +0.05，g_0 → 1.02 > f_0 0.98，翻到第一
        boosted = idx.search("x", top_k=5, sender_name="示例用户戌")
...
def _make_keyword_index():
    ...
    messages = [
        {
            "id": "m0", "text": "粗排模型选型讨论", "sender": "示例用户亥",
            "is_self": False, "chat_name": "推荐群", "chat_type": "group",
            "context_ids": ["m0"],
        },
        {
            "id": "m1", "text": "你们的粗排先是用的eenmf 后来换的", "sender": "示例用户亥",
            "is_self": False, "chat_name": "推荐群", "chat_type": "group",
            "context_ids": ["m1"],
        },
        {
            "id": "m2", "text": "后来用了双塔", "sender": "示例用户亥",
            "is_self": False, "chat_name": "推荐群", "chat_type": "group",
            "context_ids": ["m2"],
        },
    ]
    ...
    idx.msg_by_id = {m["id"]: m for m in messages}
    idx.id_to_idx = {m["id"]: i for i, m in enumerate(messages)}
    idx.sender_index = {"示例用户亥": ["m0", "m1", "m2"]}
        ...
        results = [
            {
                "score": 0.823,
                "hit_message": {
                    "chat_name": "示例用户丁", "chat_type": "single",
                    "id": "f_0", "is_self": False, "sender": "示例用户丁",
                },
                "context_messages": [
                    {"is_self": False, "sender": "示例用户丁", "text": "明天去上海吗"},
