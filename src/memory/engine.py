...
def _split_alias_string(s: str) -> List[str]:
    ...
    LLM 经常把多个别名写成 "示例别名庚、示例别名辛" 或 "Paul、阿杰" 一整串，
...
class MemoryEngine:
    ...
    def _extract_aliases_from_user_wiki(self, wiki: str, user_name: str) -> List[str]:
        ...
        关键修复：LLM 常把多个别名写成 "示例别名庚、示例别名辛" 一整串，必须按顿号/斜杠
        拆分后再逐条校验，否则整串入库会导致按子串（如 "示例别名辛"）召回失败。
        """
        aliases: List[str] = []
        marker = "## 别名"
    ...
    def search_keyword(self, keyword: str, max_chars: int = 6000, return_scored: bool = False) -> Any:
        ...
        # user wiki，把跨人关系（如示例用户甲 wiki 提到配偶示例用户丙）挤出 Top10。
        is_person_query = any(
