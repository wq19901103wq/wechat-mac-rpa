...
class TestCleanChatNameBehavior:
    ...
    def test_digit_prefix_preserved(self):
        """'10 10 林岚' 保持原样。"""
        assert LayoutParser.clean_chat_name("10 10 林岚") == "10 10 林岚"

    def test_symbol_garbage_prefix_preserved(self):
        """'！岔站 林岚 @ai' 保持原样。"""
        assert LayoutParser.clean_chat_name("！岔站 林岚 @ai") == "！岔站 林岚 @ai"

    def test_timestamp_suffix_removed(self):
        """时间戳后缀仍去除（时间戳是 UI 元素，不是昵称的一部分）。"""
        assert LayoutParser.clean_chat_name("王老板们昨天 22:26") == "王老板们"
