...
五类检测：
  A1 单 wiki 内昵称碰撞：同一 wiki 同一昵称归 ≥2 人（如示例用户甲 wiki 里"示例昵称甲"
     同时归小舅示例用户癸和二姨示例用户己）→ internal_contradiction
...
用法：
    ...
    python3 scripts/detect_wiki_contradictions.py --user 示例用户甲     # 单个
...
def _is_significant_nick(nick: str) -> bool:
    """昵称显著性过滤：≥2 字且非"示例别名庚/小张"类常见姓氏前缀词。"""
    if len(nick) < 2:
        return False
    if re.fullmatch(r"[老小大阿]\w", nick):  # 示例别名庚/小张/大李/阿强
        return False
    return True
...
#   模式2: 人名（X）  —— owner 在前，昵称在括号（如 示例用户癸（示例昵称甲））
#   模式3: X（人名）  —— 昵称在前，owner 在括号（如 示例昵称甲（示例用户癸））
