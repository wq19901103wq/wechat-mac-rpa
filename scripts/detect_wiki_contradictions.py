...
五类检测：
  A1 单 wiki 内昵称碰撞：同一 wiki 同一昵称归 ≥2 人（如林岚 wiki 里"乔家花园"
     同时归小舅林建华和二姨林梅）→ internal_contradiction
...
用法：
    ...
    python3 scripts/detect_wiki_contradictions.py --user 林岚     # 单个
...
#   模式2: 人名（X）  —— owner 在前，昵称在括号（如 林建华（乔家花园））
#   模式3: X（人名）  —— 昵称在前，owner 在括号（如 乔家花园（林建华））
