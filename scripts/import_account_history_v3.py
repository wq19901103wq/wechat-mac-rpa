#!/usr/bin/env python3
"""
纯串行版本：导入另一个微信账号的历史聊天记录到 global_state.json。
用法:
    python3 scripts/import_account_history_v3.py --account b
"""
...
def main():
    parser = argparse.ArgumentParser(description="串行导入另一个微信账号的历史聊天记录")
    ...
    print(f"=" * 50)
    print(f"串行账号导入: account='{account}'")
    ...
    for idx, contact in enumerate(contacts, 1):
        name, msgs, status = fetch_one_contact(weflow, contact, account, max_msgs)
        if msgs is None:
            skipped += 1
            if "无消息" not in status:
                errors += 1
                print(f"  [{idx}/{len(contacts)}] {contact.name}: {status}")
        else:
            new_chats[name] = msgs
            total_msgs += len(msgs)
            if len(msgs) > 100:
                print(f"  [{idx}/{len(contacts)}] {name}: {len(msgs)} 条")

        if idx % 100 == 0:
            elapsed = time.time() - start_time
            print(f"  ... 进度 {idx}/{len(contacts)}, 已用 {elapsed:.0f}s")
