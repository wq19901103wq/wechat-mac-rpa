...
- **3 个失败测试**：根因是 engine fixture 未隔离 aliases，加载真实 aliases.json 导致"程立"被 resolve 成"程立-君奕"。修法：fixture 加 `engine._aliases = {}`。非被测代码 bug。
- **wiki 截断阈值统一**：代码护栏 4000 vs lint 脚本 10000 不一致，统一到 4000，跑 truncate 清 152 个历史膨胀 wiki（白.md 14775→3986）。
- **5 个 known_issue 修 4 个**：人名查询时给 user wiki BM25 ×1.3 boost，避免 group wiki 挤占。剩 multi_wangqiaosheng 是数据缺失（王海 wiki 未提王乔生）。
...
- ✅ comp_yihan_ali 修好：王芊 rank10→rank2（召回了排序错，rerank 修）
- ❌ neg_wangqian_mother 等 3 个未修：当时以为"召回阶段漏"，**后被召回池指标修正**——秋水文章其实在召回池第13（pool=Y13），是排序问题（rerank 只看 top10 漏掉 11-30 候选），不是召回问题。
...
1. **LLM rerank 候选扩到 top30**——能捞回 pool=Y11-30 的 primary（如 neg_wangqian_mother 的秋水文章 Y13）。明确可执行。
2. **pool=N 的真召回问题**（hop_wangqian_eryi 王夏兰、comp_pudong_house 陆杰）——只能靠 query 改写/同义词扩展（妈妈→母亲），不是 rerank 能解。
3. **multi_wangqiaosheng 数据缺失**——王海 wiki 补"父亲:王乔生"（wiki 内容补全，非检索逻辑）。
