...
| `prompts/persona.md` | 删除输出格式段，改为允许思考 |
...
3. 人设符合度：是否像林岚本人？是否保持简洁、幽默、略带傲娇？是否变成 AI 助手腔？
4. 称呼正确性：是否使用对方偏好称呼？（如小夏必须叫“小夏”）
5. 极简原则：是否啰嗦、重复、废话？
...
def test_investment_question_triggers_think():
    ...
    replies = gen.generate(
        [ChatMessage(text="拼多多还能拿吗", sender="小夏", sender_type=SenderType.OTHER)],
        [],
        is_group=True,
    )
    assert replies
