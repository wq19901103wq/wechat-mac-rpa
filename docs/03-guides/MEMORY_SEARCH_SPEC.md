...
例：输入 `"林岚 同事"` → `raw_keywords = ["林岚", "同事"]` → `keywords = ["林岚", "同事"]`（假设林岚没有别名需要扩展）
...
**陷阱**: `substring_match` 用的是 `resolved_keyword in name`，不是 `name in resolved_keyword`。所以搜 "林岚" 时：
- ✅ "林岚.md" → primary（name_match）
- ✅ "林岚_2.md" → primary（substring_match: "林岚" in "林岚_2"）
- ✅ "示例用户甲、晨光、林岚.md" → primary（substring_match: "林岚" in "示例用户甲、晨光、林岚"）
- ❌ "王.md" → **不是 primary**（"林岚" not in "王"）
...
例：文档里 "同事" 出现 30 次，"林岚" 出现 1 次。如果 "同事" 在 keywords 列表中排在前面，可能提取的 2 个片段都围绕 "同事"，完全不包含 "林岚"。
...
| 问题 | 现象 | 根因 |
|------|------|------|
| Primary 过多挤占 Top 5 | 搜 "林岚 同事" 只返回林岚自己的 wiki，赵川/Alex 被挤出 | 4 个文件名含 "林岚" 的文档都被判为 primary，占了 Top 4 |
